...
Published releases ship as multi-arch (amd64 + arm64) images on GitHub
Container Registry. Skip the `--build` step entirely:

```bash
git clone https://github.com/williamdeng3244/LLM-WIKI.git
cd LLM-WIKI
cp .env.example .env
# Edit .env, optionally pin TAG=v0.1.0 (defaults to :latest)
docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d
```

Images:

- `ghcr.io/williamdeng3244/llm-wiki-backend:<tag>` — FastAPI + Celery worker (same image, different command)
- `ghcr.io/williamdeng3244/llm-wiki-frontend:<tag>` — Next.js
...
1. Builds the backend + frontend images for `linux/amd64` and `linux/arm64`.
2. Pushes them to `ghcr.io` tagged `:v0.2.0`, `:0.2.0`, `:0.2`, and `:latest`.
3. Creates a GitHub Release with autogenerated notes + the image-pull snippet.

After the very first release, open the repo's *Packages* tab → each package
→ *Package settings* → set visibility to **Public** so `docker pull` works
without auth. This is a one-time toggle per package.
